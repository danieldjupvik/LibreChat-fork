...
const ResponseCost = ({ message, conversation, isLast }: ResponseCostProps) => {
  ...
  const findModelSpec = (modelName: string, specName?: string): TModelSpec | null => {
    ...
    return modelSpecs.find(spec => 
      spec.preset?.model === modelName || 
      spec.label === modelName
    ) || null;
  };
  ...
  const getPricingFromSpec = (modelName: string, specName?: string) => {
    ...
    const inputCostPerToken = inputPrice !== undefined && inputPrice !== null
      ? inputPrice / 1000000
      : null;
      
    const outputCostPerToken = outputPrice !== undefined && outputPrice !== null
      ? outputPrice / 1000000
      : null;
  ...
  useEffect(() => {
    ...
    const calculateCost = async () => {
      try {
        ...
        const { inputCostPerToken, outputCostPerToken, isFree } = 
          getPricingFromSpec(messageModel, specName);
